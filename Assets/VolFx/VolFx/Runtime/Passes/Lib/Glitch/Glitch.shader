...
Shader "Hidden/Vol/Glitch" 
{
	SubShader 
	{
		Pass 
		{
            ...
            #pragma multi_compile_local SIDES_CLAMP SIDES_REPEAT
			...
			fixed4 frag(v2f i) : COLOR
			{
				...
				fixed4 filter = tex2D(_ColorTex, float2(noise.a, 0), 0, 1);
				filter.rgb += initial * filter.a;

#if SIDES_CLAMP
				fixed4 shot = tex2D(_ShotTex, clamp(i.uv + float2(noise.b * dir, 0), 0.001, 0.999));
#endif
#if SIDES_REPEAT
